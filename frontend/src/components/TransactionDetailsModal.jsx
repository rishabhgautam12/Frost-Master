...
export default function TransactionDetailsModal({ record, type, onClose }) {
  ...
  const downloadImage = () => {
    ...
    const paymentRows = isSale ? Math.max(1, payments.length) : 0;
    ...
    [["Dispatched Through",details.dispatchedThrough],["Destination",details.destination],["Motor Vehicle No.",details.motorVehicleNo],["Payment Mode",record.paymentMode]].forEach(([label,value],i)=>{const x=pad+18+i*(width-pad*2)/4;text(label,x,dispatchY+23,10,800,"#64748b");text(display(value),x,dispatchY+48,13,700);});
  ...
  return <Modal open wide title={`${isSale ? "Sale" : "Order"} Details - ${number}`} onClose={onClose}>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:8, background:"#ecfeff", border:"1px solid #a5f3fc", padding:12, borderRadius:8, marginBottom:12 }}>{[["Dispatched Through",details.dispatchedThrough],["Destination",details.destination],["Motor Vehicle No.",details.motorVehicleNo],["Payment Mode",record.paymentMode],["Sale Type",record.saleType]].map(([label,value])=><div key={label}><div style={{fontSize:9,fontWeight:800,color:"#64748b",textTransform:"uppercase"}}>{label}</div><strong>{display(value)}</strong></div>)}</div>
    <div style={{ overflowX:"auto", marginBottom:14 }}><table style={{ width:"100%", minWidth:760, borderCollapse:"collapse", fontSize:11 }}><thead><tr>{["Product","Description","Warehouse","Qty","Rate","Billing Rate","Discount","GST","Transport","Total"].map(h=><th key={h} style={{padding:8,background:"#e2e8f0",textAlign:"left"}}>{h}</th>)}</tr></thead><tbody>{(record.items||[]).map((item,i)=>{const total=(+item.total||0)+(+item.gstAmount||0)+(+item.transportAmount||0)+(+item.transportGstAmount||0);return <tr key={item._id||i}>{[item.productName||item.product?.name||"Product",item.description||"-",item.warehouse||"-",item.qty,money(item.rate),money(item.billingRate),`${item.discount||0}%`,`${item.gstRate||0}%`,money(item.transportAmount),money(total)].map((v,j)=><td key={j} style={{padding:8,borderBottom:"1px solid #e2e8f0"}}>{v}</td>)}</tr>})}</tbody></table></div>
    <div style={{ display:"grid", gridTemplateColumns:"1fr minmax(260px,360px)", gap:16 }}><div><strong>Notes</strong><div style={{whiteSpace:"pre-wrap",color:"#64748b",marginTop:5}}>{display(record.notes)}</div></div><div style={{background:"#f8fafc",padding:12,borderRadius:8}}>{summary.map(([label,value])=><div key={label} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",fontSize:label==="Grand Total"?15:12,borderTop:label==="Grand Total"?"1px solid #cbd5e1":"none"}}><span>{label}</span><strong>{money(value)}</strong></div>)}{isSale&&<><div style={{display:"flex",justifyContent:"space-between",padding:"5px 0",color:"#15803d"}}><span>Paid</span><strong>{money(record.amountPaid)}</strong></div><div style={{display:"flex",justifyContent:"space-between",padding:"5px 0",color:"#dc2626"}}><span>Due</span><strong>{money(record.amountDue)}</strong></div></>}</div></div>
    {isSale && <div style={{marginTop:16}}><strong>Payment History</strong>{payments.length?payments.map((p,i)=><div key={p._id||i} style={{display:"grid",gridTemplateColumns:"120px 120px 1fr 110px",gap:8,padding:9,borderBottom:"1px solid #e2e8f0"}}><span>{date(p.date)}</span><span>{p.paymentMode}</span><span>{p.notes||"-"}</span><strong style={{color:"#15803d",textAlign:"right"}}>{money(p.amount)}</strong></div>):<div style={{color:"#94a3b8",padding:10}}>No itemized payment records.</div>}</div>}
  </Modal>;
}
