...
export default function TransactionDetailsModal({ record, type, onClose }) {
  ...
  const downloadImage = async () => {
    ...
    [["E-Way Bill No.",details.eWayBillNo],["Destination",details.destination],["Motor Vehicle No.",details.motorVehicleNo],["Payment Mode",record.paymentMode]].forEach(([label,value],i)=>{const x=pad+18+i*(width-pad*2)/4;text(label,x,dispatchY+23,10,800,"#64748b");text(display(value),x,dispatchY+48,13,700);});
    ...
    (record.items || []).forEach(item=>{rect(pad,y,width-pad*2,48,"#fff","#e2e8f0");text(item.productName || item.product?.name || "Product",cols[0]+6,y+19,12,700);text(`HSN: ${item.hsnCode || item.product?.hsnCode || "-"}  ${String(item.description || "").slice(0,32)}`,cols[0]+6,y+37,10,400,"#64748b");text(display(item.warehouse),cols[1]+6,y+27,11);text(item.qty,cols[2]+6,y+27,11);text(money(item.rate),cols[3]+6,y+27,11);text(`${item.gstRate || 0}%`,cols[4]+6,y+27,11);text(money(item.transportAmount),cols[5]+6,y+27,11);const total=(+item.total||0)+(+item.gstAmount||0)+(+item.transportAmount||0)+(+item.transportGstAmount||0);text(money(total),cols[6],y+27,11,800,"#0f172a","right");y+=48;});
